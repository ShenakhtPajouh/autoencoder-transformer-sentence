...
class SelfAttention(tf.keras.layers.Layer):
    ...
    def call(self, inputs, encoded=None, cache=None, mask=None,
             attention_dropout=None, return_cache=False,
             use_2d=False, shape=None, encoded_length=None):
        """
        inputs: a tensor of shape [batch_size, seq_length, dim] if use_2d is false,
                else a tensor of shape [batch_size * seq_length, dim]
        "encoded": Tensor of shape [batch_size, encoded_length, dim] if use_2d is false,
                  else a tensor of shape [batch_size * encoded_length, dim]
        ...
        encoded_length: if use_2d is True, then the encoded length
        """
        query = self.query_layer(inputs)
        key = self.key_layer(inputs)
        value = self.value_layer(inputs)
        if encoded is not None and cache is not None:
            raise ValueError("only cache or encoded could be specified")
        if use_2d and shape is None:
            raise ValueError("if use_2d is True, then the shape must be specified")
        if use_2d and encoded is not None and encoded_length is None:
            raise ValueError("if use_2d is True, then encoded_length must be specified")
        ...
        if encoded is not None:
            encoded_key = self.key_layer(encoded)
            encoded_value = self.value_layer(encoded)
            if use_2d:
                _shape = [shape[0], encoded_length]
            else:
                _shape = None
            encoded_key = self.reshape(encoded_key, use_2d, _shape)
            encoded_value = self.reshape(encoded_value, use_2d, _shape)
            cache_length = get_tensor_shape(encoded_key)[2]
            key = tf.concat([encoded_key, key], 2)
            value = tf.concat([encoded_value, value], 2)   
    ...
    def __call__(self, inputs, encoded=None, cache=None, mask=None,
                 attention_dropout=None, return_cache=False,
                 use_2d=False, shape=None, encoded_length=None):
        """
        inputs: a tensor of shape [batch_size, seq_length, dim] if use_2d is false,
...
class EncodedProjection(tf.keras.layers.Layer):
    def __init__(self, embedding_size=768, name=None, trainable=True):
        if name is None:
            name = "encoded_projection"
        super().__init__(name=name, trainable=trainable)
...
class AttentionLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config, name=None, trainable=True, initializer_range=0.02):
        ...
        self.projection = tf.keras.layers.Dense(units=config["n_embd"],
                                                kernel_initializer=tf.random_normal_initializer(stddev=initializer_range),
                                                name="projection")
        self.encoded_projection = EncodedProjection(
            embedding_size=config["n_embd"],
            name="encoded_projection",
        )

    def call(self, inputs, encoded=None, cache=None, dropout=None, attention_dropout=None,
             return_cache=False, use_2d=False, shape=None, encoded_length=None):
        """
        ...
        """
        if encoded is not None:
            encoded = self.encoded_projection(encoded)
            encoded = dropout_fn(encoded, dropout)
        x = self.layer_norm(inputs)
    ...
    def __call__(self, inputs, encoded=None, cache=None, dropout=None, attention_dropout=None,
                 return_cache=False, use_2d=False, shape=None, encoded_length=None):
        """
        inputs: a tensor of shape [batch_size, seq_length, dim] if use_2d is False, else [batch_size * seq_length, dim]
        encoded: a tensor of shape [batch_size, encoded_length, dim] if use_2d is False, else [batch_size * encoded_length, dim]
...
class Transformer(tf.keras.Model):
    ...
    def call(self, inputs, encoded=None, cache=None, dropout=None, attention_dropout=None,
             return_cache=False, blocks=None, use_2d=False, shape=None, encoded_length=None):
        """
        inputs: a tensor of shape [batch_size, seq_length, dim], if use_2d is False, else [batch_size * seq_length, dim]
        encoded: a tensor of shape [batch_size, encoded_length, dim] if use_2d is False, else [batch_size * encoded_length, dim]
...
class Decoder(tf.keras.Model):
    ...
    def call(self, inputs, encoded=None, cache=None,
             ...
             return_cache=False, return_logits=True, use_2d=False, encoded_length=None):
        """
        ...
        if cache is not None:
            _cache = cache[0]["key"]
            start = get_tensor_shape(_cache)[2] - 1
        else:
            start = None
    ...
    def __call__(self, inputs, encoded=None, cache=None,
                 ...
                 use_2d=False, encoded_length=None):
        """
